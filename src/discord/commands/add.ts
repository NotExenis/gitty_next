...
export default {
    ...
    async execute(interaction: any) {
        ...
        if (interaction.options.getNumber("duration")) {
            let days = interaction.options.getNumber("duration");
        ...
        try {
            ...
            // Discord part
            let dmDesc = `You have recieved **1x** Auth Token for: **${product}**.\\n\\n**Your Token:** ${token}\\n\\nPut this token in your \`token.txt\` file.`;
            let successDesc = `:white_check_mark: Added ${email} to product **${product}**\\n\\nToken: \`${token}\`\\nDuration: \`${duration}\``;
